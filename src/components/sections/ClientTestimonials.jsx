...
const testimonials = [
  {
    ...
    initial: "MR",
    text: "Gunatit Enterprise did an amazing job renovating our home. The team was very supportive throughout the process and guided us at every step. The final result came out even better than we expected.",
  },
  {
    icon: "🍳",
    ...
    location: "Kandivali",
    initial: "KR",
    text: "We got our kitchen and bathroom renovated by Gunatit Enterprise, and the experience was really smooth. The designs are modern and practical, and the work was completed on time.",
  },
  {
    icon: "✨",
    service: "Marble, Granite & Vitrified Flooring",
    location: "Thane, Maharashtra",
    initial: "NP",
    text: "Choosing Gunatit Enterprise for our flooring project was the right decision. The team helped us select the perfect tiles and completed the installation with excellent craftsmanship. The polished finish has completely transformed our home's appearance.",
  },
  {
    icon: "🏢",
    ...
    location: "Juhu",
    initial: "JC",
    text: "Our commercial project was handled very professionally. Communication was clear from start to finish. They delivered on time without compromising on quality.",
  },
  {
    icon: "🎨",
    ...
    location: "Mira Road",
    initial: "MM",
    text: "We got interior and finishing work done, including POP and painting. The finishing quality was neat, and the team paid attention to small details. Overall a very good experience.",
  },
  {
    icon: "💧",
    ...
    location: "Andheri",
    initial: "AW",
    text: "We had leakage issues for a long time, and Gunatit Enterprise provided a proper waterproofing solution. It's been months now and there are no problems at all.",
  },
  {
    icon: "🌴",
    ...
    location: "Goa",
    initial: "GC",
    text: "We wanted everything managed by one team, and Gunatit Enterprise handled civil work, interiors, and electrical perfectly. It made the whole process completely stress-free.",
  },
  {
    icon: "🏡",
    service: "Premium Home Flooring",
    location: "Borivali, Mumbai",
    initial: "DK",
    text: "Finding a trustworthy tile fitting contractor wasn't easy until we found Gunatit Enterprise. Their experienced craftsmen delivered premium-quality flooring with excellent precision. The entire process was transparent, timely, and completely stress-free.",
  },
...
function Card({ t, featured }) {
  return (
    ...
    >
      ...
      {featured && (
        <span
          className="absolute top-5 right-5 bg-white/20 text-white text-[10px] font-bold
...
export default function ClientTestimonials() {
  ...
  const goIdx = (i) => {
    if (i !== active && phase === "idle") navigate(i > active ? 1 : -1);
  };
  ...
  const cardClasses = (role) => {
    const base =
      "transition-all duration-[420ms] ease-[cubic-bezier(.4,0,.2,1)] will-change-transform";
    if (phase === "idle" || phase === "enter") {
      if (role === "center")
        return `${base} opacity-100 scale-100 translate-x-0`;
      return `${base} opacity-45 scale-[0.88] translate-x-0`;
    }
    if (phase === "exit") {
      if (dir === 1) {
        if (role === "center")
          return `${base} opacity-0 scale-90 -translate-x-14`;
        if (role === "prev")
          return `${base} opacity-0 scale-[0.82] -translate-x-20`;
        if (role === "next")
          return `${base} opacity-65 scale-95 -translate-x-7`;
      } else {
        if (role === "center")
          return `${base} opacity-0 scale-90 translate-x-14`;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className={`ct-rv ct-d1 ${inView ? "in" : ""} text-center mb-14`}>
          ...
          <div className="flex justify-center mb-5">
            <div
              className="inline-flex items-center gap-2 bg-[#eff3fb] border border-[#dce5f7]
          ...
          >
            What Our <span className="ct-shimmer">Clients Say</span>
          </h2>

          <p
            className="text-slate-500 font-light max-w-[500px] mx-auto leading-relaxed mb-5"
            style={{ fontSize: "15.5px" }}
          >
            Trusted by homeowners and businesses across Mumbai — here's what
            they experienced.
          </p>
        ...
        {/* ════ PROGRESS BAR ════ */}
        <div
          className={`ct-rv ct-d2 ${inView ? "in" : ""} max-w-xl mx-auto mb-10
        ...
        >
          <div className="flex items-stretch justify-center gap-5">
            {/* LEFT */}
            <div
              className={`hidden md:block w-[230px] flex-shrink-0 ${cardClasses("prev")}`}
            >
            ...
            {/* CENTER */}
            <div
              className={`w-[440px] max-w-full flex-shrink-0 ${cardClasses("center")}`}
            >
            ...
            {/* RIGHT */}
            <div
              className={`hidden md:block w-[230px] flex-shrink-0 ${cardClasses("next")}`}
            >
        ...
        {/* ════ CONTROLS ════ */}
        <div
          className={`ct-rv ct-d3 ${inView ? "in" : ""} flex items-center justify-center gap-4 mb-3`}
        >
          {/* Prev btn — outline */}
          <button
            onClick={goPrev}
        ...
        {/* Counter */}
        <p
          className={`ct-rv ct-d3 ${inView ? "in" : ""} text-center text-[13px] font-light text-slate-400 mb-14`}
        >
          <span className="text-[#4167BE] font-bold">{active + 1}</span> /{" "}
          {TOTAL}
        </p>
      </div>
    </section>
